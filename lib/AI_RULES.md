...
## ★ PROJECT ACCESS RULE (apply before any task)

> Before starting any task or making the first file change in a session, the AI **must** ask the user for permission to access the entire project — not just the specific file mentioned.
> Briefly explain why broader access is needed (e.g., to find related files, check dependencies, or understand the full context).
> **Do NOT** proceed with multi-file edits or exploration until the user grants permission.
...
- When generating or modifying code, **always write changes directly into the MAIN project's `lib/` directory**: `/Users/mostafasamy/StudioProjects/talentlink/lib/`
...
```
✅ /Users/mostafasamy/StudioProjects/talentlink/lib/src/...   ← correct target
❌ /Users/mostafasamy/StudioProjects/talentlink/.claude/worktrees/*/lib/src/...  ← WRONG — user cannot see this
❌ .claude/suggestions.dart           ← wrong — never put code here only
❌ Only shown in chat response        ← wrong — must be written to disk
```
...
**Rule:** Never put multiple widget classes in the same file. Every widget gets its own file — including private `_` widgets.

```
// ❌ NEVER — multiple widgets in one file (even private ones)
// project_list_card.dart
class ProjectListCard { ... }
class _AvailabilityBadge { ... }   // ← private widget still violates this rule

// ✅ ALWAYS — one widget per file
presentation/screens/project_list/widget/project_list_card.dart  → ProjectListCard
presentation/widgets/availability_badge_widget.dart              → AvailabilityBadgeWidget
...
> This applies even when the widget is small or only used in one place. Extract it, name it properly (no leading `_`), and import it.
...
find.descendant(of: find.byType(Dialog), matching: find.byType(ElevatedButton))
...
- [ ] Did I put each widget in its **own file** — including small or private `_` widgets?
